...
Trading::TradeEngine::TradeOrder Trading::TradeEngine::ParseInput(const std::string& input)
{
	...
	if (!std::all_of(identifier.begin(), identifier.end(), [](const char c){ return std::isalnum(c); })) [[unlikely]]
	{
        throw std::invalid_argument("Invalid identifier passed to parseInput");
	}
	if (!std::isalpha(side)) [[unlikely]]
	{
		throw std::invalid_argument("Invalid side passed to parseInput");
	}

	if (std::all_of(quantity_str.begin(), quantity_str.end(), [](const char c){ return std::isdigit(c); })) [[likely]] 
	{
		quantity = std::stoull(quantity_str);
	}
	if (std::all_of(price_str.begin(), price_str.end(), [](const char c){ return std::isdigit(c); })) [[likely]]
	{
                                      ...
                                      const std::uint64_t price) noexcept
{
	while (quantity > 0 && !m_buyOrders.empty())
	{
		...
		if (bestBuyPrice < price) [[unlikely]]
		{
			break;
		}
...
void Trading::TradeEngine::CollectTrades() noexcept
{
    ...
    for (const auto& [identifier, sign] : m_tradeInfo)
    {
        ...
        {
            tradeOutput.emplace_back(trader + trade_sign + std::to_string(quantity) + "@" +
                               std::to_string(price));
        }
    }

	std::sort(tradeOutput.begin(), tradeOutput.end());
    if (!tradeOutput.empty()) [[likely]]
    {
        std::stringstream ss;
